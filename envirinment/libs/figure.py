...
#datas = [[[expert] for _ in n_agents] for _ in n_teration]
def heatmap_gif(datas, state_size, opt_datas=None, feature=False, labels=None, folder="./", file_name="Non", save=True, show=False):
    def make_heatmap(i):
        print(f"{i+1}/{fms}")
        for j in range(n_agent):
            ax = fig.add_subplot(fig_size[0], fig_size[1], j+1)
            ax.cla()
            data = np.array(datas[j][i])
            ...
            if labels:
                ...
                if opt_datas:
                    color = 'red' if all(opt_datas[j]==data) else 'black'
                ax.set_title(str(i), color=color)
            if not feature:
                data = calc_state_visition_count(n_state, [data])
            if len(data.shape) < 2:
                data = data.reshape(state_size)
            if arr[j]: arr[j].remove()
            ax.set_facecolor((1,0,0,0))
            arr[j] = sns.heatmap(data, ax=ax, cbar=False, square=True, annot=False,cmap='PuRd')
            ax.axis('off')

    n_agent = len(datas)
    n_iter = len(datas[-1])
    print(f"{n_agent} <= n*m")
    arr = [None for _ in range(n_agent)]
    s = input("n,m = ").split(",")
    fig_size = [int(s[0]), int(s[1])]
    n_state = state_size[0]*state_size[1]
    fms = n_iter if n_iter<=128 else np.linspace(0, n_iter-1, 128).astype(int)
